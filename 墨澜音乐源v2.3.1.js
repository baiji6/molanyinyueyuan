...
const TX_BACKENDS = [
  ...
  // === 前端1: Hello World QQ（a.aa.cab/qq.music，取自Hell World音源260809主API） ===
  {
    name: 'HelloWorld QQ',
  ...
  // === 后端10: nki.pw API（flac用） ===
  {
    ...
    fetch: async (songmid, quality) => {
      ...
      for (const key of apiKeys) {
        try {
          const res = await httpFetch(_u('cookn5**\\kd)ifd)kr*<KD*hpnd^Zjk`iZ\\kd)kck:hd_8') + songmid + '&apikey=' + key, {
            method: 'GET', timeout: 10000,
            headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
          })
          const d = res.body
  ...
  // === 后端11: tang.api.s01s.cn（flac用） ===
  {
    ...
    fetch: async (songmid, quality) => {
      ...
      const res = await httpFetch(_u('cookn5**o\\ib)\\kd)n+,n)^i*hpnd^Zjk`iZ\\kd)kck:hd_8') + songmid, {
        method: 'GET', timeout: 10000,
        headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
      })
      const d = res.body
  ...
  // === 后端15: 长青SVIP 海棠直链 ===
  {
    name: '长青直链',
    fetch: async (songmid, quality) => {
      const res = await httpFetch(_u('cook5**,20)-2),11)-.1*fbll,*ll)kck:otk`8hk.!d_8') + songmid + '&level=' + quality, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0' },
      })
      const d = res.body
  ...
  // === 后端18: GD Studio API ===
  {
    ...
    fetch: async (songmid, quality) => {
      ...
      const res = await httpFetch(_u('cookn5**hpnd^(\\kd)b_nop_dj)stu*\\kd)kck:otk`n8pmg!njpm^`8ll!d_8') + songmid + '&br=' + br, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
      })
      const d = res.body
  ...
  // === 后端23: 汽水VIP API ===
  {
    ...
    fetch: async (songmid, quality) => {
      ...
      const res = await httpFetch(_u('cookn5**\\kd)qn\\\\)^i*\\kd*hpnd^)ldncpd)qdk:\\^o8njib!d_8') + songmid + '&quality=' + level, {
        method: 'GET', timeout: 10000,
        headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
      })
      const d = res.body
...
const WY_BACKENDS = [
  ...
  // === 后端6: GD Studio API ===
  {
    ...
    fetch: async (songmid, quality) => {
      const brMap = { '128k': '128', '320k': '320', flac: '740', flac24bit: '999', hires: '999' }
      const br = brMap[quality] || '128'
      const res = await httpFetch(_u('cookn5**hpnd^(\\kd)b_nop_dj)stu*\\kd)kck:pn`Zs]md_b`.8omp`!gj\\_`mZi\\h`8ajm`no!i``_Zn`^Zgdif8,!n`^ZgdifZn^`i`8dh!oc`h`8gdbco!otk`n8pmg!njpm^`8i`o`\\n`!d_8') + songmid + '&br=' + br, {
        method: 'GET', timeout: 10000,
        headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
      })
      const d = res.body
  ...
  // === 后端11: 长青SVIP 直链 ===
  {
    name: '长青直链',
    fetch: async (songmid, quality) => {
      const res = await httpFetch(_u('cook5**,20)-2),11)-.1*rt,*rt)kck:otk`8hk.!d_8') + songmid + '&level=' + quality, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0' },
      })
      const d = res.body
...
const KW_BACKENDS = [
  ...
  // === 后端6: 长青直链 ===
  {
    ...
    fetch: async (songmid, quality) => {
      ...
      const res = await httpFetch(_u('cook5**,20)-2),11)-.1*fbll,*fr)kck:otk`8hk.!d_8') + songmid + '&level=' + level, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0' },
      })
      const d = res.body
...
const KG_BACKENDS = [
  ...
  // === 后端3: 长青直链（175.27.166.236，备用） ===
  {
    ...
    fetch: async (songmid, quality, musicInfo) => {
      ...
      const res = await httpFetch(_u('cook5**,20)-2),11)-.1*fbll,*fb)kck:otk`8hk.!d_8') + hash + '&level=' + level, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0' },
      })
      const d = res.body
  ...
  // === 后端16: GD Studio API ===
  {
    ...
    fetch: async (songmid, quality) => {
      const brMap = { '128k': '128', '320k': '320', flac: '740', flac24bit: '999', hires: '999' }
      const br = brMap[quality] || '128'
      const res = await httpFetch(_u('cookn5**hpnd^(\\kd)b_nop_dj)stu*\\kd)kck:otk`n8pmg!njpm^`8fb!d_8') + songmid + '&br=' + br, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
      })
      const d = res.body
...
const MG_BACKENDS = [
  ...
  // === 后端4: GD Studio API ===
  {
    ...
    fetch: async (songmid, quality) => {
      ...
      const br = brMap[quality] || '128'
      const res = await httpFetch(_u('cookn5**hpnd^(\\kd)b_nop_dj)stu*\\kd)kck:otk`n8pmg!njpm^`8hdbp!d_8') + songmid + '&br=' + br, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
      })
      const d = res.body
  ...
  // === 后端7: 星海后端（Hei Music xhbackend） ===
  {
    ...
    fetch: async (songmid, quality) => {
      const level = qualityToLevel(quality)
      const res = await httpFetch(_u('cookn5**\\kd)sdibc\\d(]\\^f`i_)^i*hdbp:d_8') + encodeURIComponent(String(songmid)) + '&quality=' + level, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' },
      })
      const d = res.body
  ...
  // === 后端8: 长青直链（haitangw） ===
  {
    ...
    fetch: async (songmid, quality) => {
      const level = qualityToLevel(quality)
      const res = await httpFetch(_u('cookn5**hpnd^)c\\do\\ibr)^^*hpnd^\\kd*hb)kck:otk`8hk.!d_8') + encodeURIComponent(String(songmid)) + '&level=' + level, {
        method: 'GET', timeout: 8000,
        headers: { 'User-Agent': 'Mozilla/5.0' },
      })
      const d = res.body
...
console.log('[QQ音乐+网易云音乐+酷我+酷狗+咪咕聚合音源 v4.4.2] 已加载完成')
