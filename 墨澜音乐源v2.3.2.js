...
const WY_BACKENDS = [
  ...
  // === 后端10: HYWmusic API（白姬专用，103.79.184.97） ===
  {
    name: 'HYWmusic',
    fetch: async (songmid, quality) => {
...
const KW_BACKENDS = [
  ...
  // === 后端11: 溯音酷我（oiapi.net，搜索式API，不依赖songmid） ===
  {
    name: '溯音酷我',
    fetch: async (songmid, quality, musicInfo) => {
  ...
  // === 后端12: Hello World KW API（lxmusic.xn--fiqs8s，带SHA256签名） ===
  {
    name: 'HelloWorld',
    fetch: async (songmid, quality, musicInfo) => {
...
const KG_BACKENDS = [
  ...
  // === 后端7: Hello World KG API（lxmusic.xn--fiqs8s，带SHA256签名） ===
  {
    name: 'HelloWorld',
    fetch: async (songmid, quality, musicInfo) => {
  ...
  // === 后端9: 海棠API（使用KG_LEVEL_MAP修正master音质） ===
  {
    name: '海棠API',
    fetch: async (songmid, quality, musicInfo) => {
      const level = KG_LEVEL_MAP[quality] || 'standard'
...
const MG_BACKENDS = [
  ...
  // === 后端7: HYWmusic API（白姬专用，103.79.184.97） ===
  {
    name: 'HYWmusic',
    fetch: async (songmid, quality) => {
